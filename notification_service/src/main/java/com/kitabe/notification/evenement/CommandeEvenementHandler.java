...
public class CommandeEvenementHandler {
     ...
     */
    public CommandeEvenementHandler(
            NotificationServices notificationServices, CommandeEvenementRepository commandeEvenementRepository) {
    ...
    void handleCreerCommandeEvenement(CreerCommandeEvenement evenement) {
        ...
        if (commandeEvenementRepository.existsByEvenementId(evenement.evenementId())) {
            log.warn("Nous avons detecté une duplication lors de la creation devenement commande pour ID"
    ...
    void handleDelivrerCommandeEvenement(CommandeDelivrerEvenement evenement) {
        log.info(
                "Réception d'un événement de livraison de commande : commandeNum={}, evenementId={}",
                evenement.commandeNum(),
                evenement.evenementId());
        if (commandeEvenementRepository.existsByEvenementId(evenement.evenementId())) {
            log.warn("Nous avons detecté une duplication lors de la livraison devenement commande pour ID"
                    + evenement.evenementId());
            return;
        }
    ...
    void handleAnnulerCommandeEvenement(CommandeAnnuleeEvenement evenement) {
        log.info(
                "Réception d'un événement d'annulation de commande : commandeNum={}, evenementId={}",
                evenement.commandeNum(),
                evenement.evenementId());

        if (commandeEvenementRepository.existsByEvenementId(evenement.evenementId())) {
            log.warn(
                    "Duplication détectée lors de l'annulation de l'événement de commande pour evenementId={}",
                    evenement.evenementId());
            return;
        }
    ...
    void handleErreurCommandeEvenement(CommandeErreurEvenement evenement) {
        log.info(
                "Réception d'un événement d'erreur de commande : commandeNum={}, evenementId={}",
                evenement.commandeNum(),
                evenement.evenementId());
        if (commandeEvenementRepository.existsByEvenementId(evenement.evenementId())) {
            log.warn(
                    "Duplication détectée lors du traitement de l'erreur de l'événement de commande pour evenementId={}",
                    evenement.evenementId());
            return;
        }
