...
import com.kitabe.kitabe_webapp.commande.CommandeSommaire;
import com.kitabe.kitabe_webapp.commande.web.utils.TestDataFactory;
...
@Sql("/test-commandes.sql")
public class CommandeControllerTest extends AbstractIT {
    ...
    class CreerCommandeTests {
        ...
        void shouldretourbadRequestWhenMandatoryDataIsMissing() {
            var payload = TestDataFactory.creerRequestCommandeAvecClientInvalide();
            given().contentType(ContentType.JSON)
                    .header("Authorization","Bearer" +getToken())
                    .body(Optional.ofNullable(payload)) // Envoie le payload JSON
                    .when()
                    .post("/api/commande") // Appelle l'endpoint POST
                    .then()
    ...
    class getCommandebyCommandeNum {
        ...
        @Test
        void shouldretourneCommandeReussi() {
            given().when().header("Authentication", "Bearer " + getToken())
